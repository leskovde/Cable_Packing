...
**Browse**: select a file from the file explorer, the file should contain the radii of wires (in millimetres, with dot as decimal separator), one per line. 
...
**Run**: Executes the selected algorithm on the chosen input file. The output is shown on the canvas below. The radii of the circles are printed at their centre. The diameter of  the wire bundle is written in the control panel, below the Browse button. 
